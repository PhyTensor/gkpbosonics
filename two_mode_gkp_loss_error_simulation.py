...
def _(mo):
    mo.md(
        r"""
        # Simulating Loss on a Two-Mode GKP Qubit
...
@app.cell
def _():
    basename: str = "two_mode_gkp_loss_"
    return (basename,)
...
def _():
    ...
    from strawberryfields.ops import (GKP, BSgate, Coherent, LossChannel,
                                      ...
                                      Zgate, Dgate, S2gate, CXgate, CZgate)
...
@app.cell
def _(BSgate, GKP, LossChannel, MeasureX, Program, np):
...
def _():
    ...
    # circuit_gkp_x = Program(2)
    # with circuit_gkp_x.context as qx:
    #     GKP(epsilon=0.0631) | qx[0]
    #     GKP(epsilon=0.0631) | qx[1]
    #     BSgate(theta=np.pi/4, phi=0) | (qx[0], qx[1])
    #     CXgate(0.5) | (qx[0], qx[1])
    #     # q_meas = 
    #     MeasureHomodyne(phi=0, select=True) | qx[0]
    #     # Dgate(q_meas) | qx[0]
    #     # MeasureX | qx[0]
...
def _():
    ...
    # circuit_gkp_p = Program(2)
    # with circuit_gkp_p.context as qp:
    #     GKP(epsilon=0.0631) | qp[0]
    #     GKP(epsilon=0.0631) | qp[1]
    #     BSgate(theta=np.pi/4, phi=0) | (qp[0], qp[1])
    #     MeasureHomodyne(phi=np.pi/2, select=True) | qp[1]
    #     Dgate(q_meas * 1) | qp[1]
    #     # MeasureP | qp[0]
    # # eng = Engine("bosonic")
